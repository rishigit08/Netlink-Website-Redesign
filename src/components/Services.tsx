...
/* Simple abstract icons for each service */
      ...
      <div className="mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3 lg:gap-6">
          {services.map((svc, i) => (
            <article
              key={svc.title}
              className="group rounded-2xl border border-slate-800 bg-slate-900/60 p-6 transition-all duration-300 hover:border-slate-700 hover:bg-slate-900/80 hover:shadow-lg hover:shadow-black/20 sm:p-8"
            >
              {/* Icon */}
              <div className="mb-5 inline-flex rounded-xl bg-blue-500/10 p-3 text-blue-400 transition-colors group-hover:bg-blue-500/15">
                {serviceIcons[i]}
              </div>
