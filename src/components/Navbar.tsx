          ...
          {navItems.map((item) => (
            ...
            >
              ...
              {item.hasDropdown && item.dropdownItems && openDropdown === item.label && (
                <div className="absolute left-0 top-full mt-2 w-[640px] rounded-xl border border-slate-700 bg-[#0b1120] p-2 shadow-2xl backdrop-blur-xl">
                  <div className="grid grid-cols-2 gap-1">
                    {item.dropdownItems.map((dropItem) => (
                      <a
                        key={dropItem.href}
                        href={dropItem.href}
                        className="block rounded-lg px-4 py-3 transition-colors hover:bg-slate-800/50"
                      >
                ...
                <div className="ml-4 mt-2 space-y-1">
                  {item.dropdownItems.map((dropItem) => (
                    <a
                      ...
                      href={dropItem.href}
                      onClick={() => setMobileOpen(false)}
                      className="block rounded-lg px-4 py-2 text-sm text-slate-400 transition-colors hover:bg-slate-800 hover:text-slate-200"
                    >
                      {dropItem.label}
                    </a>
                  ))}
